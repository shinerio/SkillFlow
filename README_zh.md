...
| **跨工具同步** | 推送支持左侧分类范围（全部 / 指定分类）或手动卡片选择，也可从 Claude Code、OpenCode、Codex、Gemini CLI、OpenClaw 及自定义工具拉取；逐条冲突处理 |
| **仓库收藏** | 关注 Git 仓库，无需导入即可浏览和使用其中的 Skills；支持文件夹或平铺视图；可直接批量推送到工具 |
| **云端备份** | 将 Skill 库镜像至阿里云 OSS、腾讯云 COS、华为云 OBS 或任意 Git 仓库；所有服务商的备份范围统一为应用数据目录（排除 `cache/` 与 `.git/`），且备份页可浏览完整远端文件列表；Git 模式会自动迁移旧的嵌套备份元数据，确保真实 Skill 文件可以被正常跟踪 |
| **更新检测** | 自动检测 GitHub 来源 Skills 的新提交；一键更新 |
| **应用自动更新** | 启动时横幅提示新版本；Windows 支持一键下载并重启；macOS 链接至 GitHub Releases 页面 |
| **托盘驻留** | 点击窗口关闭按钮仅隐藏窗口不退出；macOS 驻留顶部状态栏并使用原生单击展开菜单，Windows 驻留系统托管区并显示应用自身图标（托盘菜单可退出） |
| **设置** | 按工具独立配置启用状态、推送路径和扫描路径（文字输入 + 文件夹选择器）、固定兜底分类 `Default`（用于未分类拉取/导入）、运行期日志级别（`debug`/`info`/`error`，默认 `error`）与一键打开日志目录、自定义工具、云服务凭据、代理设置 |

每个按钮、对话框和交互的完整说明，请查阅 **[feature_zh.md](feature_zh.md)**。

侧边栏还提供了“意见反馈”入口，会在浏览器中打开 GitHub 新建 Issue 页面。
